...
import { buildSvgBezierPath, buildUsageSeries } from "@/lib/dashboard-utils";
...
export function UsageChart({ requests = [], onRefresh, isRefreshing = false }: UsageChartProps) {
  ...
  const path = buildSvgBezierPath(points);

  const firstPoint = points[0] || { x: 0, y: 100 };
  const lastPoint = points[points.length - 1] || { x: 100, y: 100 };
  const closedPath = `${path} L ${lastPoint.x.toFixed(2)} 100 L ${firstPoint.x.toFixed(2)} 100 Z`;
    ...
    >
      ...
      <div className="mt-6 h-[220px] rounded-2xl border border-[var(--border-subtle)] bg-cream-2/45 p-4">
        <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="h-full w-full overflow-visible">
          ...
          <path d={closedPath} fill="url(#dashboardUsageFill)" />
          <motion.path
            d={path}
            fill="none"
            stroke="var(--coral)"
