...
export async function getClientHash(): Promise<string> {
  ...
  const cached = localStorage.getItem("clco_bot_client_hash");
  ...
  try {
    ...
    try {
      ...
      if (ctx) {
        ...
        ctx.fillStyle = "#069";
        ctx.fillText("clco_chatbot_fingerprint", 2, 15);
        ctx.fillStyle = "rgba(102, 204, 0, 0.7)";
        ctx.fillText("clco_chatbot_fingerprint", 4, 17);
    ...
    localStorage.setItem("clco_bot_client_hash", hash);
    return hash;
  } catch (e) {
    ...
    try {
      ...
      localStorage.setItem("clco_bot_client_hash", fallback);
      return fallback;
    }
  }
}
